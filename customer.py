...
class Cust_Win:
 ...
 # variables
    ...
    #    cust name
        ...
        txtMobile=ttk.Entry(labelframeleft,textvariable=self.var_mobiles,width=29,font=("arial",13,"bold"))
        ...
        combo_Nationality["value"]=("Nepali","Other")
 ...
 #    idproof
        ...
        combo_id["value"]=("Citizenship","DrivingLicence","passport")
        combo_id.current(0)
       ...
       # btn
        ...
        Table_frameleft=LabelFrame(self.root,bd=2,relief=RIDGE,text="View Details And Search System",font=("times new roman",12,"bold"),padx=2,)
        ...
        combo_Search=ttk.Combobox(Table_frameleft,textvariable=self.search_var,font=("arial",12,"bold"),width=15,state="readonly")
        combo_Search["value"]=("Mobile" ,"Ref")
        combo_Search.current(0)
        combo_Search.grid(row=0,column=1,padx=2)
        
        self.txt_search=StringVar()
        txtsearch=ttk.Entry(Table_frameleft,textvariable=self.txt_search,width=24,font=("arial",10,"bold"))
        txtsearch.grid(row=0,column=2,padx=2)

        btnsearch=Button(Table_frameleft,text="Search",command=self.search,font=("arial",11,"bold"),bg="black",fg="gold",width=9)
        btnsearch.grid(row=0,column=3,padx=1)

        btnShowAll=Button(Table_frameleft,text="Show All",command=self.fetch_data,font=("arial",11,"bold"),bg="black",fg="gold",width=9)
    ...
    def add_data(self):
        if self.var_mobiles.get()==""or self.var_mother.get()=="":
    ...
    def update(self):
        if self.var_mobile.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="2002",database="management")
            my_cursor=conn.cursor()
            my_cursor.execute("update customer set Name=%s,Mother=%s,Gender%s,PostCode=%s,Mobile=%s,Email=%s,Nationality=%s,Idprrof=%s,Idnumber=%s,Address=%s where Ref=%s",(
            ...
            self.fetch_data()
            conn.close()
            messagebox.showinfo("Update","Customer detail has been update successfully",parent=self.root)
