...
# floor
    ...
    def update(self):
        if self.var_floor.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="2002",database="management")
            my_cursor=conn.cursor()
            my_cursor.execute("update details set Floor=%s,RoomType=%s where RoomNo=%"(
                                                                                  self.var_floor.get(),
                                                                                  self.var_RoomType.get(),
                                                                                  self.var_roomNo.get()
            ...
            self.fetch_data()
            conn.close()
            messagebox.showinfo("Update","New Room detail has been update successfully",parent=self.root)
